...
Mocks Server v2.x introduced the concept of ["route variants"](versioned_docs/version-2.6.0/get-started-routes.md), and made more restrictive the way for organizing mocks and routes files. It also introduced other improvements, as the possibility of [using express middlewares](versioned_docs/version-2.6.0/guides-using-middlewares.md), made simpler the [core API](versioned_docs/version-2.6.0/api-mocks-server-api.md), etc.

All of these changes made very difficult to handle old v1 `behaviors` and `fixtures` together with the new format of `routes`, so, in order to maintain backward compatibility, Mocks Server v2 can handle two folders at a time, one containing v1.x `fixtures` and `behaviors`, and another one containing v2.x [`mocks`](versioned_docs/version-2.6.0/get-started-mocks.md) and [`routes`](versioned_docs/version-2.6.0/get-started-routes.md).
...
After reading the v2.x [`routes`](versioned_docs/version-2.6.0/get-started-routes.md) and [`mocks`](versioned_docs/version-2.6.0/get-started-mocks.md) documentation, you would have noticed that old fixtures having the same "path" can be easily migrated to a new `route`.
...
Next [`mocksServer`](versioned_docs/version-2.6.0/api-mocks-server-api.md) methods and getters are provided only for backward compatibility, and will be removed in next major version:
