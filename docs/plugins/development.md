...
* __`core.loadCollections(collections)`__: Load [`collections`](usage/collections.md). Each time this method is called, __all previously loaded collections will be replaced by the new ones, but only those added by this plugin__. Collections loaded by the core or by other plugins will remain.
* __`core.loadRoutes(routes)`__: Load [`routes`](usage/routes.md). Each time this method is called, __all previously loaded routes will be replaced by the new ones, but only those added by this plugin__. Routes loaded by the core or by other plugins will remain.
